<template>
    <!-- pages/my-supply/my-supply.wxml -->
    <view class="my-supply-page">
        <!-- 数据统计栏 -->
        <view class="header-section">
            ...
            <view class="search-row">
                <input
                    ...
                    type="text"
                    confirm-type="search"
                    placeholder="搜索我的供应：产品 / 分类 / 规格 / 产地"
        ...
        >
            ...
            <view v-if="filteredList.length === 0 && !loading" class="empty-state">
                ...
                <text class="empty-text">您还没有发布任何供应</text>
                <button class="publish-btn" @tap="goToPublish">去发布</button>
            </view>

            <!-- 供应列表 -->
            <view v-else class="supply-list">
                <view class="supply-card" :data-id="item.id" @tap="goToEdit" v-for="(item, index) in filteredList" :key="index">
                    <view class="card-content">
                        <!-- 左侧图片 -->
                        <view class="card-image-wrapper">
                            <image class="card-image" :src="item.images[0] || '/static/images/logo.png'" mode="aspectFill"></image>
                        </view>

                        <!-- 右侧信息（标题行右上角 badge 与「我的采购」一致） -->
                        <view class="card-info">
                            <view class="card-title-row">
                                ...
                                <view class="card-title-meta">
                                    ...
                                    <view v-if="item._sv && (item._sv.promoTopActive || item._sv.promoBoostActive)" class="promo-badge-stack">
                                        <view v-if="item._sv.promoTopActive" class="meta-badge meta-badge-top">置顶</view>
                                        <view v-if="item._sv.promoBoostActive" class="meta-badge meta-badge-boost">加急推广中</view>
                                    </view>
                                </view>
                            </view>
                    ...
                    <view class="card-actions" @tap.stop.prevent="">
                        ...
                        <button class="action-btn delete-btn" :data-id="item.id" @tap.stop.prevent="deleteSupply">删除</button>
                    </view>
                </view>
...
<script>
// pages/my-supply/my-supply.js
import { PROCUREMENT_CATEGORIES } from '../../utils/constants.js';
import { getMySupplies, updateSupplyStatus, deleteSupply } from '../../utils/api.js';
...
export default {
    ...
    methods: {
        ...
        // 加载供应列表
        async loadSupplyList(refresh = false) {
            if (this.loading) {
                return;
            }
            ...
            try {
                const result = await getMySupplies({
                    page: page,
                    pageSize: this.pageSize
                });

                const newList = refresh ? result.list : [...this.supplyList, ...result.list];
                ...
                const formattedList = newList.map((item) => {
                    ...
                    const norm = normalizeSupplyItem({ ...item, id: item.id || item._id });
                    return {
                        ...norm,
                        status: status,
        ...
        // 计算统计数据
        calculateStats() {
            const stats = calculateListStats(this.supplyList);
            this.setData({
                stats
            });
        },
        ...
        // 筛选
        filterItems() {
            const { filterStatus, supplyList, categoryFilter, searchQuery } = this;
            let filtered = supplyList.filter((item) => {
            ...
            filtered.sort((a, b) => {
                const ta = a._sv && a._sv.promoTopActive ? 1 : 0;
                const tb = b._sv && b._sv.promoTopActive ? 1 : 0;
        ...
        goToEdit(e) {
            ...
            uni.navigateTo({
                url: `/pages/edit-supply/edit-supply?id=${id}`
            });
        },

        goToPromotion(e) {
            const id = e.currentTarget.dataset.id;
            const item = this.supplyList.find((s) => s.id === id);
            if (!item) return;
            const vc = getViewCount(item);
            ...
            uni.navigateTo({
                url: `/pages/promotion-center/promotion-center?content_id=${encodeURIComponent(
                    String(id)
                )}&content_type=supply&title=${title}&view_count=${vc}&published_at=${encodeURIComponent(item.publishTime || '')}`
            });
        },
        ...
        async toggleStatus(e) {
            ...
            uni.showModal({
                title: '确认操作',
                content: `确定要${action}这条供应信息吗？`,
                success: async (res) => {
                    if (res.confirm) {
                        try {
                            // 调用API更新服务器状态
                            await updateSupplyStatus(id, newStatus);
                            // 更新本地状态
        ...
        updateItemStatus(id, status) {
            const supplyList = this.supplyList.map((item) => {
                if (item.id === id) {
                    return {
                        ...item,
                        status: status,
        ...
        // 移除项目
        removeItem(id) {
            const supplyList = this.supplyList.filter((item) => item.id !== id);
            this.setData({
                supplyList
            });
            this.calculateStats();
