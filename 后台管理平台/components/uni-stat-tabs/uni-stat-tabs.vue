...
<script>
	// #ifdef H5
	import {
		waitForUniCloudUserContext
	} from '@/js_sdk/uni-admin/unicloudAuth.js'
	// #endif
	export default {
		name: "uni-stat-tabs",
		data() {
		...
		methods: {
			init() {
				if (this.mode.indexOf('platform') > -1) {
					...
					// #ifdef H5
					// 须晚于登录态写入 storage，否则会匿名请求 uni-stat-app-platforms 并产生未捕获的 Promise 拒绝
					waitForUniCloudUserContext(10000).then((ok) => {
						if (ok) {
							this.getPlatform()
						}
					})
			...
			getPlatform() {
				...
				db.collection('uni-stat-app-platforms')
					...
					.catch((err) => {
						console.warn('[uni-stat-tabs] 加载平台列表失败', err && (err.message || err.errMsg || err))
					})
			},
