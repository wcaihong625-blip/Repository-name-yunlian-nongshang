  ...
  <view class="page-shell">
    ...
    <view class="table-card">
      <uni-table border stripe :loading="loading" emptyText="暂无数据">
        <uni-tr>
          ...
          <uni-th align="center" width="90">会员类型</uni-th>
          <uni-th align="center" width="100">套餐周期</uni-th>
          <uni-th align="center" width="80">场景</uni-th>
        ...
        <uni-tr v-for="row in list" :key="row._id">
          ...
          <uni-td align="center">{{ memberTypeScopeLabel(row) }}</uni-td>
          <uni-td align="center">{{ planTypeScopeLabel(row) }}</uni-td>
          <uni-td align="center">{{ scopeLabel(row.use_scene || row.scope) }}</uni-td>
    ...
    <uni-popup ref="genPopup" type="center">
      <view class="gen-card">
        ...
        <uni-forms label-width="120">
          ...
          <uni-forms-item label="适用场景">
            <uni-data-select v-model="gen.scope" :localdata="genScopeOpts" />
          </uni-forms-item>
          <uni-forms-item label="会员类型">
            <uni-data-checkbox v-model="gen.apply_member_types" multiple :localdata="memberTypeOpts" />
          </uni-forms-item>
          <uni-forms-item label="套餐周期">
            <uni-data-checkbox v-model="gen.apply_plan_types" multiple :localdata="planTypeOpts" />
          </uni-forms-item>
    ...
    <uni-popup ref="detailPopup" type="center">
      <view class="gen-card detail-card" v-if="detailRow">
        ...
        <view class="detail-line"><text class="k">会员类型</text><text class="v">{{ memberTypeScopeLabel(detailRow) }}</text></view>
        <view class="detail-line"><text class="k">套餐周期</text><text class="v">{{ planTypeScopeLabel(detailRow) }}</text></view>
        <view class="detail-line"><text class="k">适用场景</text><text class="v">{{ scopeLabel(detailRow.use_scene || detailRow.scope) }}</text></view>
...
export default {
  data() {
    return {
      ...
      gen: {
        ...
        max_use_count: 1,
        max_use_per_user: 1,
        apply_member_types: ['all'],
        apply_plan_types: ['all'],
        remark: ''
      },
      genTypeOpts: [
      ...
      memberTypeOpts: [
        { value: 'all', text: '全部' },
        { value: 'personal', text: '个人会员' },
        { value: 'enterprise', text: '企业会员' }
      ],
      planTypeOpts: [
        { value: 'all', text: '全部' },
        { value: 'month', text: '月卡' },
        { value: 'quarter', text: '季卡' },
        { value: 'year', text: '年卡' }
      ],
      detailRow: null
    };
  ...
  methods: {
    ...
    normalizeScopeArray(value, fallbackField) {
      const raw = Array.isArray(value)
        ? value
        : String(fallbackField || value || 'all').split(',').map((x) => x.trim()).filter(Boolean);
      if (!raw.length || raw.includes('all')) return ['all'];
      return raw;
    },
    memberTypeScopeLabel(row) {
      const list = this.normalizeScopeArray(row.apply_member_types, row.member_type_scope);
      const m = { all: '全部', personal: '个人', enterprise: '企业' };
      return list.map((x) => m[x] || x).join('+');
    },
    planTypeScopeLabel(row) {
      const list = this.normalizeScopeArray(row.apply_plan_types, row.plan_type_scope);
      const m = { all: '全部', month: '月卡', quarter: '季卡', year: '年卡' };
      return list.map((x) => m[x] || x).join('+');
    },
    ruleText(row) {
      if (!row) return '';
      if (row.coupon_type === 'free') return '实付0元';
      if (row.coupon_type === 'amount') return `减${row.discount_amount || row.discount_value}元`;
      if (row.coupon_type === 'discount') return `${(Number(row.discount_rate || row.discount_value) * 10).toFixed(1)}折`;
      return '';
    },
    openGen() {
    ...
    async submitGen() {
      ...
      if (this.gen.coupon_type === 'amount' && Number(this.gen.discount_value) <= 0) {
        uni.showToast({ title: '减免金额必须大于0', icon: 'none' });
        return;
      }
      if (this.gen.coupon_type === 'discount') {
        let rate = Number(this.gen.discount_value);
        if (rate > 1 && rate <= 10) rate = rate / 10;
        if (rate <= 0 || rate > 1) {
          uni.showToast({ title: '折扣请输入0.9或9折这类数值', icon: 'none' });
          return;
        }
        this.gen.discount_value = rate;
      }
      this.genLoading = true;
      try {
        const memberTypes = this.normalizeScopeArray(this.gen.apply_member_types, 'all');
        const planTypes = this.normalizeScopeArray(this.gen.apply_plan_types, 'all');
        const body = await this.memberCouponCo().generate({
          ...this.tokenPayload(),
          ...this.gen,
          use_scene: this.gen.scope,
          apply_member_types: memberTypes,
          apply_plan_types: planTypes,
          member_type_scope: memberTypes.includes('all') ? 'all' : memberTypes.join(','),
          plan_type_scope: planTypes.includes('all') ? 'all' : planTypes.join(',')
        });
        if (body.code !== 200) {
    ...
    _downloadCouponCsv(rows, filename) {
      const header = ['code', 'batch_name', 'batch_id', 'coupon_type', 'discount_value', 'discount_amount', 'discount_rate', 'member_type_scope', 'plan_type_scope', 'scope', 'use_scene', 'status', 'used_count', 'max_use_count'];
