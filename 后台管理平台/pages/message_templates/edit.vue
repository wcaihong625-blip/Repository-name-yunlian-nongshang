  ...
  <view class="page-container">
    <view class="page-header">
      <view class="header-title">
        ...
        <text class="title-text">编辑消息模板</text>
      </view>
      <view class="header-subtitle">修改已有的消息通知配置</view>
    </view>
        ...
        <view class="form-section">
          ...
          <uni-forms-item name="remark" label="备注说明">
            <uni-easyinput
              type="textarea"
              autoHeight
              placeholder="说明可使用的占位符，例如：{{username}}、{{shopName}} 等"
  ...
  methods: {
    ...
    getDetail(id) {
      ...
      db.collection(dbCollectionName)
        .doc(id)
        .field('name,code,channel,title,content,enabled,remark,create_time,update_time')
        .get()
        .then(res => {
          const data = (res.result && res.result.data && res.result.data[0]) || (res.data && res.data[0])
          if (data) {
            this.formData = data
          }
        })
