...
<template>
	<view class="uni-content">
		...
		<uni-captcha
			...
			@update:modelValue="handleCaptchaInput"
		/>
		<!-- 带选择框的隐私政策协议组件 -->
	...
	export default {
		...
		methods: {
			handleCaptchaInput(value) {
				this.captcha = typeof value === 'string' ? value.trim() : ''
			},
			// 页面跳转，找回密码
			toRetrievePwd() {
			 ...
			 */
			pwdLogin() {
				const captchaValue = typeof this.$refs.captcha?.getValue === 'function'
					? this.$refs.captcha.getValue().trim()
					: this.captcha.trim()
				...
				if (this.needCaptcha && captchaValue.length != 4) {
