...
exports.main = async (event, context) => {
  ...
  if (event.source_channel_id) queryList.push({ source_channel_id: String(event.source_channel_id).trim() });
  ...
  try {
    ...
    for (const item of allData) {
      idForCode.push(item.first_sales_id, item.current_sales_id);
    }
    const codeMap = await batchSalesCodeByStaffId(db, idForCode);
    ...
    const list = allData.map(item => {
      ...
      const fsid = item.first_sales_id ? String(item.first_sales_id).trim() : '';
      const csid = item.current_sales_id ? String(item.current_sales_id).trim() : '';
      return {
        mobile: item.mobile || '',
        nickname: item.nickname || '',
        first_sales_code: fsid ? codeMap[fsid] || '' : '',
        first_sales_name: item.first_sales_name || '待分配',
        current_sales_code: csid ? codeMap[csid] || '' : '',
    ...
    const headers = [
      'mobile', 'nickname', 'first_sales_code', 'first_sales_name', 'current_sales_code', 'current_sales_name', 'source_channel_name',
    ...
    const headers_zh = [
      '手机号', '昵称', '首次业务员编号', '首次归属业务员', '当前业务员编号', '当前服务业务员', '来源渠道',
