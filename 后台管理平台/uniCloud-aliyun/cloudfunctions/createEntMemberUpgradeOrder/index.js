...
/**
 * 企业类型升级（个人 → 企业）：order_type=3，备注 biz_type=upgrade_enterprise。
 * 云函数名须 ≤30 字符（阿里云限制），故使用 createEntMemberUpgradeOrder。
 */
...
exports.main = async (event, context) => {
  ...
  try {
    ...
    if (!quote.ok) {
      return res(400, quote.message || '无法创建升级订单');
    }
    ...
    const pay_amount = quote.upgrade_pay;
    const original_amount = quote.enterprise_price;
    const discount_amount = quote.discount_amount;
    const rights = rightsForTierAndPlan(mpcfg, 'enterprise', quote.plan_key) || {};
    ...
    const remarkObj = {
      ...
      biz_type: 'upgrade_enterprise',
      from_member_tier: 'personal',
      personal_price_snapshot: quote.personal_price,
      enterprise_price_snapshot: quote.enterprise_price,
      remaining_value: quote.remaining_value,
      upgrade_pay: quote.upgrade_pay,
      remaining_ratio: quote.remaining_ratio,
      biz_rule_hint:
        '企业类型升级：按剩余时间与同档企业价折算应付；支付成功后到期不变，仅类型与权益为企业会员。非周期顺延。'
    };
    ...
    console.error('createEntMemberUpgradeOrder error:', err);
